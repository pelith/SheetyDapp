...
class JSBI extends Array {
  ...
  toString(radix = 10) {
    ...
    if ((radix & (radix - 1)) === 0) {
      return JSBI.__toStringBasePowerOfTwo(
        this,
        radix,
      );
    }
    return JSBI.__toStringGeneric(
      this,
      radix,
      false,
    );
  ...
  static divide(x, y) {
    ...
    if (JSBI.__absoluteCompare(
      x,
      y,
    ) < 0) return JSBI.__zero();
    ...
    if (y.length === 1 && divisor <= 0xFFFF) {
      ...
      quotient = JSBI.__absoluteDivSmall(
        x,
        divisor,
        null,
      );
    } else {
      quotient = JSBI.__absoluteDivLarge(
        x,
        y,
        true,
        false,
  ...
  static remainder(x, y) {
    ...
    if (y.length === 1 && divisor <= 0xFFFF) {
      ...
      const remainderDigit = JSBI.__absoluteModSmall(
        x,
        divisor,
      );
    ...
    const remainder = JSBI.__absoluteDivLarge(
      x,
      y,
      false,
      true,
  ...
  static leftShift(x, y) {
    ...
    if (y.sign) {
      return JSBI.__rightShiftByAbsolute(
        x,
        y,
      );
    }
    return JSBI.__leftShiftByAbsolute(
      x,
      y,
    );
  }
  ...
  static bitwiseAnd(x, y) {
    if (!x.sign && !y.sign) {
      ...
      // (-x) & (-y) == ~(x-1) & ~(y-1) == ~((x-1) | (y-1))
      // == -(((x-1) | (y-1)) + 1)
      let result = JSBI.__absoluteSubOne(
        x,
        resultLength,
      );
      const y1 = JSBI.__absoluteSubOne(y);
      result = JSBI.__absoluteOr(
        result,
        y1,
        result,
      );
    ...
    // x & (-y) == x & ~(y-1) == x &~ (y-1)
    return JSBI.__absoluteAndNot(
      x,
      JSBI.__absoluteSubOne(y),
    ).__trim();
  }

  static bitwiseXor(x, y) {
    if (!x.sign && !y.sign) {
      return JSBI.__absoluteXor(
        x,
        y,
      ).__trim();
    } if (x.sign && y.sign) {
      ...
      const result = JSBI.__absoluteSubOne(
        x,
        resultLength,
      );
      const y1 = JSBI.__absoluteSubOne(y);
  ...
  static asIntN(n, x) {
    ...
    if (!hasBit) {
      return JSBI.__truncateToNBits(
        n,
        x,
      );
    }
    if (!x.sign) {
      return JSBI.__truncateAndSubFromPowerOfTwo(
        n,
        x,
        true,
      );
  ...
  static __toStringGeneric(x, radix, isRecursiveCall) {
    ...
    if (conqueror.length === 1 && divisor <= 0xFFFF) {
      ...
      const divisionResult = JSBI.__absoluteDivLarge(
        x,
        conqueror,
        true,
        true,
      ...
      secondHalf = JSBI.__toStringGeneric(
        remainder,
        radix,
        true,
      );
    ...
    quotient.__trim();
    let firstHalf = JSBI.__toStringGeneric(
      quotient,
      radix,
      true,
    );
  ...
  static __compareToBigInt(x, y) {
    ...
    const result = JSBI.__absoluteCompare(
      x,
      y,
    );
  ...
  static __multiplyAccumulate(
    ...
    for (let i = 0; i < multiplicand.length; i++, accumulatorIndex++) {
      ...
      const m1High = m1 >>> 16;
      const rLow = JSBI.__imul(
        m1Low,
        m2Low,
      );
      const rMid1 = JSBI.__imul(
        m1Low,
        m2High,
      );
      const rMid2 = JSBI.__imul(
        m1High,
        m2Low,
      );
      const rHigh = JSBI.__imul(
        m1High,
        m2High,
      );
  ...
  static __internalMultiplyAdd(source, factor, summand, n, result) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      const rx = JSBI.__imul(
        digit & 0xFFFF,
        factor,
      );
      const r0 = (rx & 0xFFFF) + high + carry;
      carry = r0 >>> 16;
      const ry = JSBI.__imul(
        digit >>> 16,
        factor,
      );
    ...
    } else if (carry + high !== 0) throw new Error('implementation bug');
  }
  ...
  __inplaceMultiplyAdd(multiplier, summand, length) {
    ...
    for (let i = 0; i < length; i++) {
      ...
      const dLow = d & 0xFFFF;
      const dHigh = d >>> 16;
      const pLow = JSBI.__imul(
        dLow,
        mLow,
      );
      const pMid1 = JSBI.__imul(
        dLow,
        mHigh,
      );
      const pMid2 = JSBI.__imul(
        dHigh,
        mLow,
      );
      const pHigh = JSBI.__imul(
        dHigh,
        mHigh,
      );
  ...
  static __absoluteDivLarge(dividend, divisor, wantQuotient, wantRemainder) {
    ...
    if (shift > 0) {
      divisor = JSBI.__specialLeftShift(
        divisor,
        shift,
        0, /* add no digits */
      );
    }
    const u = JSBI.__specialLeftShift(
      dividend,
      shift,
      1, /* add one digit */
    );
  ...
  static __leftShiftByAbsolute(x, y) {
    ...
    if (bitsShift === 0) {
      ...
      } else if (carry !== 0) throw new Error('implementation bug');
    }
    return result.__trim();
  }
